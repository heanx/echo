...
- 评论 `comments`
- 歌词 `lyrics`
- 用户资料与账户安全 `core.UserProfile`
- SQLite WAL 连接配置
- demo 数据命令 `seed_demo`
- 首页、注册、登录、资料设置、账户安全、音乐列表、上传、详情、评论页、歌词页、歌词上传页
- 播放量上报接口和基础回归测试

当前版本已经达到大作业“初步可用”阶段：本地运行后可完成注册登录、资料编辑、账户安全维护、浏览、上传、播放、歌词、评论、播放量统计等核心演示流程。
...
/login/                   -> core.views.login_view, name="login"
/register/                -> core.views.register_view, name="register"
/logout/                  -> core.views.logout_view, name="logout"
/settings/profile/        -> core.views.profile_settings, name="profile_settings"
/settings/security/       -> core.views.account_security, name="account_security"
/users/<username>/        -> core.views.profile_view, name="profile"
```
...
账户使用 Django 内置 `auth.User`：

```text
username  -> Echo ID，登录名和个人主页地址
email     -> 可选邮箱
password  -> Django 哈希密码，不明文保存
```
...
- `EchoUserCreationForm`：注册、ID 唯一性、密码规则、头像上传/预设头像。
- `UserProfileForm`：昵称、头像、简介。
- `AccountSecurityForm`：ID 和密码。
...
修改密码时必须校验当前密码，保存后调用 `update_session_auth_hash()` 保持会话。
...
- 优先返回 `user.profile.avatar_url`。
- `avatar_url` 会处理上传头像和系统预设头像。
- 没有头像时返回空字符串，由模板使用首字母 fallback。
...
- `phase_3_account.md` 是账户、资料、头像、安全中心专题文档。
...
- 注册页字段已统一为“昵称”和“ID”。
- 注册和账户安全中心都会检测 ID 重复。
- 密码规则统一为不低于 8 位、包含英文字母和数字、仅允许指定字符。
- 注册页确认密码实时显示一致 / 不一致，并使用自定义密码可见性按钮。
- 资料编辑页只负责昵称、头像、简介。
- 本人资料卡片新增“账户安全中心”入口，可修改 ID 和密码。
- 顶部账户菜单改为全局设置菜单，已移除不符合 Echo 的支持 / 下载入口。
...
- 真实用户登录系统核心已落地；评论点赞、播放历史、通知等仍需继续绑定用户维度。
...
- `openPlaybackResource()` 会拦截底部歌词/评论同次点击，直接调用 `loadMainResource()` 请求当前播放 track 的歌词或评论。
- `loadMainResource()` 使用 `AbortController` 和请求版本号，防止旧请求晚返回覆盖新内容。
