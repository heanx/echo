...
截至 2026-05-15，当前版本已经从“大作业初步可用”推进到“主要播放与搜索闭环已落地”阶段：可以运行本地服务、查看音乐社区首页、上传音频、浏览音乐列表、进入歌词/评论页、播放音频、记录播放量、使用独立搜索系统、通过右键菜单管理前端播放队列，并通过基础回归测试。
...
- 搜索：顶部搜索框输入时显示建议浮层，提交后进入 `/search/` 独立结果页。
...
- `core.views.search_view()`
- `core.views.search_suggest_view()`
- 路由：
  - `/search/`
  - `/search/suggest/`
- 模板：
  - `templates/search/results.html`
  - `templates/search/type_tabs.html`
  - `templates/search/type_summary.html`
  - `templates/search/type_tracks.html`
  - `templates/search/type_albums.html`
  - `templates/search/type_users.html`
  - `templates/search/track_result_row.html`
...
5. `docs/frontend_dynamic_resources.md`
   - 动态封面和响应式布局约定。
6. `docs/todo_next_steps_2026_05_10.md`
   - 当前 TODO、已完成项、新 GUI/登录/搜索/管理后台规划。
7. `docs/Echo 极简音频分享社区 GUI 设计规格说明书.md`
   - 新 GUI 响应式规范和可交互原型落地规则。
8. `docs/阶段说明书_2026_05_11.md`
   - 当前大作业阶段成果说明。
9. `docs/phase_1_frontend_setup.md`
   - 早期工程底座文档，仅作为背景参考。
...
- `core/views.py`
  - `home()`、`lyrics()`、`comments()` 已接数据库查询；`search_view()` 和 `search_suggest_view()` 负责搜索页与输入建议。
- `core/urls.py`
  - `/`、`/lyrics/`、`/comments/`。
- `comments/models.py`
- `comments/queries.py`
...
- 折叠按钮在未 hover 时已经预偏移到左侧并透明，hover 后滑入；标题只在 hover 时右移，避免默认状态看起来歪。
...
- 右侧播放列表现在由 `static/js/echo-shell.js` 的 `renderPlaylistQueue()` 根据 `playQueue` 重绘，计数来自 `playQueue.length`。
- 服务端仍可提供首屏 fallback 曲目，但真实上一首、下一首、播放结束、右键菜单操作都以 `playQueue` 为准。
...
- 当前共有 45 个 Django 测试，最近一次运行全部通过。
...
- 开发环境媒体文件由 `core.views.serve_media_range()` 响应，支持 `Range` 请求。不要把 `echo_project/urls.py` 里的 DEBUG media 路由改回默认静态 serving，否则部分音频 seek 会退回 `0:00`。
...
- `static/js/echo-shell.js` 维护 `playQueue`、`playQueueIndex`、`playQueueName`，并持久化到 localStorage。
- 右侧播放列表来源是 `playQueue`，由 `renderPlaylistQueue()` 重绘到 `#playlist-track-list`。
- 页面中的 `[data-play-queue]` 容器描述当前列表来源，内部 `[data-echo-track]` 行或卡片会被 `resolveQueueContext()` 收集为播放队列。
- 音乐库、最新上传、搜索结果、顶部搜索建议、详情页、右侧播放列表都可以为播放器建立明确队列。
- 上一首、下一首、随机播放池和 `audio ended` 事件都基于 `playQueue` 运行；单曲循环同步到原生 `audio.loop`。
- `[data-echo-track]` 支持右键菜单：立即播放、下一首播放、加入播放队列、从播放队列移除、查看歌曲。
