...
- 播放量上报接口已完成；前端播放队列已由 `playQueue` + localStorage 驱动，后续如需要多端同步再补登录用户维度的服务端播放历史和播放队列模型。
...
- 用户系统已落地基础注册、登录、资料和上传者绑定；评论作者、点赞用户、播放历史仍可继续完善。
- 搜索系统已使用 ORM `icontains` 完成课程阶段功能；后续数据量变大后再考虑全文索引。
...
- 歌词页按 `track` 参数读取指定歌曲。
- 搜索页已按歌曲、专辑、用户做 ORM 检索，并接入分类分页。
- 右侧播放列表已由前端 `playQueue` 驱动，`shell_queue_tracks` 仅作为首屏 fallback。
- `tests/test_views.py` 共有 45 个基础回归测试。

下一阶段需要新增或扩展的数据能力：

- 评论作者、评论回复 UI、评论点赞接口。
- 歌单 CRUD、歌单详情页和整单播放。
- 播放历史/播放队列：从当前前端 localStorage 队列升级为登录用户维度的服务端数据。
