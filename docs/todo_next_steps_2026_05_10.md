...
- 已完成：
  ...
  - 注册页已统一字段命名：昵称用于展示，ID 用于登录和个人主页地址
  - 注册与账户安全中心均已检测 ID 重复
  - 密码规则已统一：不低于 8 位，必须包含英文字母和数字，可使用指定符号
  - 注册页已支持确认密码实时一致 / 不一致提示和自定义密码显示按钮
  - 资料编辑只保留昵称、头像、简介；ID 和密码拆入账户安全中心
  - 本人个人主页卡片已新增“账户安全中心”入口，可修改 ID 和密码
  - 顶部账户下拉已调整为全局设置菜单，去掉不符合项目的支持 / 下载入口
